...
const CurrencyConverterSection = () => {
  ...
  const [comparisonData, setComparisonData] = useState(
    getComparisonData(83.12, "USD", 100)
  );
        ...
        <div className="p-6 md:p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className="bg-theme-purple/10 text-theme-purple border-theme-purple/20 px-3 py-1 flex items-center gap-1.5"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-between items-center py-2 px-3 rounded-md bg-theme-blue/5 border border-theme-blue/10">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 text-theme-blue ${
            ...
            <div className="space-y-2">
              <Label
                htmlFor="converted"
                className="block text-sm font-medium text-theme-gray"
              >
                They Receive
              </Label>
              <div className="flex gap-3">
      ...
      <div className="animate-fade-in-up">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-300 max-w-3xl mx-auto">
            ...
            when sending {amount} {sourceCurrency.code}.
          </p>
        </div>
        ...
        <div className="overflow-x-auto bg-gradient-to-br from-indigo-950/70 to-theme-dark/90 backdrop-blur-sm border border-indigo-500/20 rounded-xl shadow-lg">
          <table className="w-full min-w-full divide-y divide-white/10">
            <thead>
              <tr>
                <th className="py-4 px-6 text-left text-sm font-medium text-indigo-300">
                  Provider
                </th>
                <th className="py-4 px-6 text-left text-sm font-medium text-indigo-300">
                  Exchange Rate (1 {sourceCurrency.code})
                </th>
                <th className="py-4 px-6 text-left text-sm font-medium text-indigo-300">
                  They Receive (for {amount} {sourceCurrency.code})
                </th>
                <th className="py-4 px-6 text-left text-sm font-medium text-indigo-300">
                  Fees
                </th>
            ...
            <tbody className="divide-y divide-white/10">
              {comparisonData.map((item, index) => (
                ...
                >
                  <td
                    className={`py-4 px-6 ${
                      item.highlight ? "text-indigo-400 font-semibold" : ""
                    }`}
                  >
                    {item.provider}
                  </td>
          ...
          <div className="p-4 text-xs text-gray-400">
            *Exchange rates and fees are for comparison purposes only and may
            vary based on amount and payment method.
          </div>
        </div>
      </div>
    </div>
