...
const RateComparisonSection = () => {
  ...
  const MobileComparisonCard = ({
    ...
    <div
      className={`mb-4 p-4 rounded-lg border border-white/10 ${
        item.highlight
          ? "bg-gradient-to-r from-theme-blue/10 to-theme-cyan/10"
          : "bg-theme-dark/50"
      }`}
    >
      <h3
        className={`text-lg font-medium mb-3 ${
          item.highlight ? "text-theme-cyan font-space" : ""
        }`}
      >
        {item.provider}
      </h3>
  ...
  return (
    <section id="rate_comp" className="py-16 relative overflow-hidden">
      ...
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          ...
          <p className="text-gray-300 max-w-3xl mx-auto text-lg">
            See how our service stacks up against traditional remittance
            options.
          </p>
        </div>
              ...
              {isMobile ? (
                <div className="p-4">
                  ...
                  <div className="text-xs text-gray-400 mt-4">
                    *Exchange rates and fees are for comparison purposes only
                    and may vary based on amount and payment method.
                  </div>
                </div>
              ) : (
                <>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-b border-white/10">
                          <TableHead className="text-theme-cyan py-6 text-base font-space">
                            Service
                          </TableHead>
                          <TableHead className="text-theme-cyan py-6 text-base font-space">
                            Exchange Rate
                          ...
                          >
                            <TableCell
                              className={`font-medium py-6 ${
                                item.highlight
                                  ? "text-theme-cyan text-lg font-space"
                                  : ""
                              }`}
                            >
                              {item.provider}
